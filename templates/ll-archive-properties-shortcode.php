    ...
    <label for="state">State</label>
      <select class="state" name="state" id="state">
        ...
        $filter_posts = new WP_Query(array(
          ...
          foreach (array_unique($state_arr) as $current_state) { ?>
            <option value="<?php echo $current_state;?>"<?php
            
            //If state has been set then set it as selected on front end
            if (isset($state) && $state == $current_state) {
              echo 'selected';
            } else {
              echo '';
            }
            
            ?>><?php echo $current_state; ?>
          </option>
          <?php
          }
          wp_reset_postdata();
...
<?php
  ...
  //Loop through the query and return the specified property posts
